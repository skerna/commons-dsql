...
import io.skerna.dsql.core.*
import ddl.CreateTableStatement
import ddl.DropTableStatement
import dml.*

object PostgresDialect : io.skerna.dsql.core.Dialect {
    ...
    override fun <T : Table> build(statement: CreateTableStatement<T>): String {
        ...
        for (definition in statement.definitions) {
            ...
            if (definition is PostgresDefinition) {
    ...
    private fun appendProjection(builder: StringBuilder, projection: Iterable<Projection>, fullFormat: Boolean) {
        if ("SELECT".equals(builder.toString().trim()) and projection.none()) {
            builder.append("*")
        } else {
            var delim = ""
            for (proj in projection) {
                ...
                delim = ", "
                when(proj){
                    is Table.FunctionColun -> {
                        if (fullFormat) {
                            appendFullColumnNameFunction(builder, proj)
                        } else {
                            appendShortColumnNameFunction(builder, proj)
                        }
                    }
                    is Table.Column -> {
                        if (fullFormat) {
    ...
    private fun appendFullColumnNameFunction(builder: StringBuilder, column: Table.FunctionColun) {
        val function = column.apply()
        builder.append("\"${column.table}\".\"$column\"$function")
    }

    private fun appendShortColumnNameFunction(builder: StringBuilder, column: Table.FunctionColun) {
        val function = column.apply()
        builder.append("\"$column\"$function")
    }
